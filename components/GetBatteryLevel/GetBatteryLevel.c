...
esp_err_t battery_get_average_voltage(int samples, float *out_voltage)
{
    ...
    float average_voltage = total_voltage / valid_samples;
    *out_voltage = average_voltage / VOLTAGE_DIVIDER_RATIO;  // 考虑分压比
    return ESP_OK;
}

// ========== 获取电池图标字符串 ==========
const uint8_t * battery_pattern(float voltage)
{
    if (voltage >= 4.0f) {
        return battery_pattern_5;  // 满电
    } else if (voltage >= 3.7f) {
        return battery_pattern_4;  // 高电
    } else if (voltage >= 3.4f) {
        return battery_pattern_3;  // 中电
    } else if (voltage >= 3.0f) {
        return battery_pattern_2;  // 低电
    } else {
        return battery_pattern_1;  // 极低电
    }
}